...
function checkSchema(schema,root,depth=0) {
  ...
  const allowed=new Set(['$schema','$id','$ref','$defs','definitions','title','description','default','examples','type','enum','const','anyOf','oneOf','allOf','properties','required','additionalProperties','items','minItems','maxItems','minLength','maxLength','pattern','minimum','maximum','exclusiveMinimum','exclusiveMaximum','multipleOf']);
  ...
  for(const key of ['minItems','maxItems','minLength','maxLength'])if(schema[key]!==undefined&&(!Number.isInteger(schema[key])||schema[key]<0))throw invalid(`JSON Schema ${key} 无效`);
...
export function parseStructured(content,policy) {
  ...
  let value;try{value=JSON.parse(content);}catch{throw failed('模型没有返回有效 JSON');}
