...
export class ResponseStore {
  constructor(maximum=128,ttl=60*60*1000){
    ...
    this.maximum=maximum;this.ttl=ttl;this.entries=new Map();
  }
  prune(now=Date.now()){
  ...
  get(id){
    this.prune();const entry=this.entries.get(id);if(!entry)throw failure(404,`响应不存在或已过期：${id}`);
    return structuredClone(entry.value);
  }
  delete(id){
    this.prune();if(!this.entries.delete(id))throw failure(404,`响应不存在或已过期：${id}`);
  }
