...
Chat 支持 `max_tokens`、`max_completion_tokens`、`stream_options.include_usage`、`tools`、`tool_choice`、`response_format`、常用采样参数和 `net_go`；Kimi K3 的采样参数按官方范围校验。Responses 支持进程内 `store:true`、`previous_response_id`、`background:true`、状态轮询、取消、读取、删除和输入项查询；默认保存一小时，服务重启后清空。
