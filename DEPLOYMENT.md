...
| Linux amd64 | Docker 或 Node.js | CI 已配置，待运行 |
| 树莓派 arm64（64 位系统） | Docker 或 Node.js | 多架构 CI 已配置，待运行 |
...
- 自动登录目前有 mock 覆盖，尚未用真实 CAS 账号验证。验证码或登录页变更会报错，不会绕过人工验证。
...
Responses 和 Anthropic Messages 的文本、流式及工具适配已通过本地 HTTP 测试；图片、独立推理块和完整客户端兼容仍待完善。工具调用尚未通过真实模型验证。Docker 双架构镜像尚未构建验证，GHCR 尚未发布。
